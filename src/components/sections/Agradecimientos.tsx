      ...
      <div className="space-y-16">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
          <div className="p-6 sm:p-8 border-b border-gray-100">
            ...
            <p className="text-imgenia-blue font-semibold text-sm">
              Susana Jiménez Aibar — Tercera Teniente de Alcalde, Concejal de Economía, Desarrollo Empresarial y Empleo
            </p>
          </div>
          <div className="aspect-video w-full max-w-3xl mx-auto p-4 sm:p-6 bg-gray-50">
            <iframe
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {testimonios.map((t) => (
            <div
              key={t.empresa}
              className="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm hover:shadow-md transition-shadow flex flex-col"
            >
              <Quote className="w-10 h-10 text-imgenia-blue/30 mb-4 flex-shrink-0" />
              <blockquote className="text-gray-700 leading-relaxed flex-1 mb-6">
                &ldquo;{t.texto}&rdquo;
              </blockquote>
              <div className="border-t border-gray-100 pt-4">
                <p className="font-bold text-gray-900">{t.nombre}</p>
                <p className="text-sm text-imgenia-blue font-medium">{t.cargo}</p>
                <p className="text-sm text-gray-500 mt-0.5">{t.empresa}</p>
              </div>
            </div>
          ))}
        </div>
